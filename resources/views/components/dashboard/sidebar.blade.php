<div class="col-2 p-3 text-bg-dark d-flex flex-column sidebar">
    ...
    <ul class="nav nav-pills flex-column mb-auto">
        @foreach ($menu as $m)
            @if (!isset($m['role']) || in_array(Auth::user()->role, $m['role']))
                @if (!isset($m['child']))
                    <li class="nav-item">
                            ...
                            <ul class="btn-toggle-nav list-unstyled fw-light small">
                                @foreach ($m['child'] as $child)
                                    <li><a href="{{ $child['route'] }}"
                                            class="nav-link text-white">{{ $child['name'] }}</a></li>
                                @endforeach
                            </ul>
                        </div>
                    </li>
    ...
    <hr>
    <small class="fw-lighter">SIBAJA | Sistem Informasi Pembayaran dan Belanja</small>
</div>
